...
def get_data_loader(batchSize):
    ...
    # Prepare data loader
    data_loader = DataLoader(full_trainset, batch_size=batchSize,
                              shuffle=True, num_workers=0)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *block(latent_dim+class_num, 128, normalize=False), # Asterisk('*') in front of block means unpacking list of layers - leave only values(layers) in list
            *block(128, 256),
            *block(256, 512),
            *block(512, 1024),
    ...
    def forward(self, z, c): # z == latent vector(random input vector)
        
        x = torch.cat([z, c], 1)
        img = self.model(x) # (64, 100) --(model)--> (64, 784)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(img_size*img_size+class_num, 512), # (28*28, 512)
    ...
    def forward(self, img, c):
        ...
        inputs = torch.cat([img, c], dim=1)
        img_flat = inputs.view(inputs.size(0), -1) #flatten -> from (64, 1, 28, 28) to (64, 1*28*28)
...
def train(num_epochs, trainGenerator, trainDiscriminator, optimizer_G, optimizer_D, adversarial_loss, train_loader):
    ...
    for epoch in range(num_epochs): # suggested default = 200
        ...
        for i, (data, data_label) in enumerate(train_loader):
            ...
            data_label = torch.eye(10)[data_label].to(device)
            ...
            gen_label = torch.randint(low=0, high=10, size=(data.size(0),), dtype=torch.int32).to(device)
            gen_label = torch.eye(10)[gen_label].to(device)
            gen_data = trainGenerator(z,gen_label)
            g_loss = adversarial_loss(trainDiscriminator(gen_data,gen_label), valid)
            g_loss.backward()
            optimizer_G.step()
            ...
            real_loss = adversarial_loss(trainDiscriminator(real_data, data_label), valid) # torch.nn.BCELoss() compare result(64x1) and valid(64x1, filled with 1)
            fake_loss = adversarial_loss(trainDiscriminator(gen_data.detach(),gen_label), fake) # We are learning the discriminator now. So have to use detach()                                                                
            ...
            sample_labels_in_train = gen_label
            classes=torch.argmax(sample_labels_in_train, dim=1)

            # Generate samples from the generator
            sample_gen_imgs_in_train = trainGenerator(sample_z_in_train, sample_labels_in_train).detach().cpu()
            
            if ((i+1) % 700) == 0 and visual==True: # show while batch - 200/657, 400/657, 600/657
                ...
                for ncol in range(ncols):
                    ...
                    axes[ncol].set_title(f'Class: {classes[ncol]}')
                    axes[ncol].axis('off')
                plt.show()
        ...
        print(
            " [D loss: %f] [G loss: %f] [total loss %f]"
            % (d_loss.item(), g_loss.item(), d_loss.item() + g_loss.item())
        )
        
def generate(generator,size):
    generator.eval()
    sample_z_in_train = torch.randn(size, latent_dim)
    gen_label = torch.randint(low=0, high=10, size=(size,), dtype=torch.int32).to(device)
    gen_label = torch.eye(10)[gen_label].to(device)
    sample_labels_in_train = gen_label
    classes=torch.argmax(sample_labels_in_train, dim=1)

    # Generate samples from the generator
    sample_gen_imgs_in_train = generator(sample_z_in_train, sample_labels_in_train).detach().cpu()
            
    
                
    nrow=1
    ncols=10
    fig, axes = plt.subplots(nrows=nrow,ncols=ncols, figsize=(8,3))
    plt.suptitle('Insert title here')
    for ncol in range(ncols):
        axes[ncol].imshow(sample_gen_imgs_in_train.permute(0,2,3,1)[ncol], cmap='gray')
        axes[ncol].set_title(f'{classes[ncol]}')
        axes[ncol].axis('off')
    plt.show()
